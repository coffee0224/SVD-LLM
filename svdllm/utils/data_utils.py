...
def get_calib_train_data(
    name, tokenizer, nsamples, seqlen=2048, seed=3, batch_size=1, dataset_cache_dir=None
):
    import random

    random.seed(seed)
    cache_file = f"cache/{name}_{nsamples}_{seqlen}_{seed}_{batch_size}.pt"
    nsamples += 1  #############################
    ...
    if name == "c4":
        ...
        traindata = load_dataset(
            "ptb_text_only", "penn_treebank", split="train", cache_dir=dataset_cache_dir
        )
        tot_text = "\n\n".join(traindata["sentence"])
    elif name == "wikitext2":
        traindata = load_dataset(
            "wikitext", "wikitext-2-raw-v1", split="train", cache_dir=dataset_cache_dir
        )
        tot_text = "\n\n".join(traindata["text"])
    else:
...
def get_wikitext2(nsamples, seed, seqlen, tokenizer, dataset_cache_dir=None):
    ...
    testdata = load_dataset(
        "wikitext", "wikitext-2-raw-v1", split="test", cache_dir=dataset_cache_dir
    )
...
def get_ptb_new(nsamples, seed, seqlen, tokenizer, dataset_cache_dir=None):
    ...
    testdata = load_dataset(
        "ptb_text_only", "penn_treebank", split="test", cache_dir=dataset_cache_dir
    )
